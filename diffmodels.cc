...
void DTI::linfit(){
  ...
  for (int i=1;i<=npts; i++){
    if(Y(i)>0){logS(i)=log(Y(i));}
    else{logS(i)=0;}
  }
  m_dvec = -iAmat*logS;
  if(m_dvec(7)>-23){m_s0=exp(-m_dvec(7));}
  else{m_s0=Y.MaximumAbsoluteValue();}
  ...
  //////////
  m_dvec = -iAmat*logS;
  m_sse = (Amat*m_dvec+logS).SumSquare();
  m_pred = exp(-Amat*m_dvec);
  m_s0=exp(-m_dvec(7));
...
void DTI::gmmfit(){
  ...
  ////////// GMM step
  m_sqrweights = IdentityMatrix(Y.Nrows());
  m_dvec    = pinv(m_sqrweights*(-Amat))*(m_sqrweights*logS);
  m_res     = logS+(m_sqrweights*Amat)*m_dvec;


  ColumnVector tmpvec = abs(m_res-median(m_res));
  float C = 1.4826*median(tmpvec);
  for(int i=1;i<=m_res.Nrows();i++){
    m_sqrweights(i,i) = 1/(m_res(i)*m_res(i)+C*C);
  }
  m_dvec    = pinv(m_sqrweights*(-Amat))*(m_sqrweights*logS);
  m_res     = logS+(m_sqrweights*Amat)*m_dvec;
...
void PVM_multi::fit(){
  ...
  if(m_d_std>m_d)m_d_std=m_d; // is this reasonable?
