		...
		puts $log "set probtrack($entry) $probtrack($entry)"
	       ...
	       simple { 
		    ...
		    puts $log "set probtrack(units) $probtrack(units)"
               ...
               seedmask {
		     if { $probtrack(bcyn) } { 
                       fdt_monitor_short $w "${FSLDIR}/bin/flirt -in $probtrack(bedpost_dir)/nodif_brain_mask -ref $probtrack(bedpost_dir)/nodif_brain_mask -applyisoxfm 5 -out $probtrack(output)/lowresmask"
